...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
  ...
  public void initcontext(Context ctx) {
    ...
    public static String[] getCrashReportFiles(Context ctx) {
        File filesDir = new File(getCrashFilePath());
        String[] fileNames = filesDir.list();
        int length = fileNames.length;
        String[] filePaths = new String[length];
        for (int i = 0; i < length; i++) {
            filePaths[i] = getCrashFilePath() + fileNames[i];
        }
        return filePaths;
    }
...
//        sb.append("\nAPPLICATION_ID:").append(BuildConfig.APPLICATION_ID);//软件APPLICATION_ID
        sb.append("\nVERSION_CODE:").append(android.os.Build.VERSION.RELEASE);//软件版本号
        //设备信息
        ...
        try {
            FileWriter writer = new FileWriter(getCrashFilePath() + now + CRASH_REPORTER_EXTENSION);
