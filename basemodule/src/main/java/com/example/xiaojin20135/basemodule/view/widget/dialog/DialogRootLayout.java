...
import com.example.xiaojin20135.basemodule.util.ui.WindowHelper;
...
public class DialogRootLayout extends ViewGroup {
    ...
    public DialogRootLayout(@NonNull Context context, @NonNull DialogView dialogView, @Nullable FrameLayout.LayoutParams dialogViewLp) {
        super(context);
        mDialogView = dialogView;
        if (dialogViewLp == null) {
            dialogViewLp = new FrameLayout.LayoutParams(LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        }
        mDialogViewLp = dialogViewLp;
        addView(mDialogView, dialogViewLp);
        mMinWidth = ResHelper.getAttrDimen(context, R.attr.dialog_min_width);
        mMaxWidth = ResHelper.getAttrDimen(context, R.attr.dialog_max_width);
        mInsetHor = ResHelper.getAttrDimen(context, R.attr.dialog_inset_hor);
        mInsetVer = ResHelper.getAttrDimen(context, R.attr.dialog_inset_ver);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mCheckKeyboardOverlay) {
            Rect visibleInsetRect = WindowHelper.unSafeGetWindowVisibleInsets(this);
            Rect contentInsetRect = WindowHelper.unSafeGetContentInsets(this);
        ...
        if (mDialogViewLp.height > 0) {
            ...
            if (keyboardOverlayHeight > 0) {
                ...
                childMaxHeight = Math.min(heightSize - 2 * mInsetVer - contentInsetVer,
                        (int) (DisplayHelper.getScreenHeight(getContext()) * mMaxPercent - 2 * mInsetVer));
            }
        ...
        setMeasuredDimension(mDialogView.getMeasuredWidth(), mDialogView.getMeasuredHeight() + 2 * mInsetVer + keyboardOverlayHeight + contentInsetVer);
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        mDialogView.layout(childLeft, mInsetVer, childLeft + mDialogView.getMeasuredWidth(), mInsetVer + mDialogView.getMeasuredHeight());
    }
